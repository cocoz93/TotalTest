...
[Phase 4: 잠재 결함 점검 (비파괴)]
  ...
  4) Release() 이중 호출
     ...
     소멸자도 Release() 를 부른다. 명시 호출 후 소멸하면 이중 해제.
     → 현재 외부 호출처 없음(잠재). 계약: Release() 를 직접 부르지 말 것

  5) Initialize() 재호출
     이전 _Buff 를 해제하지 않고 덮어쓴다. 두 번 부르면 첫 버퍼가 누수.
     → 현재 외부 호출처 없음(잠재). 계약: 생성 시 1회만
